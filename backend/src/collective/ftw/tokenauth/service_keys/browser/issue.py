...
from collective.ftw.tokenauth.service_keys.browser.base_form import BaseForm
from collective.ftw.tokenauth.service_keys.browser.base_form import IKeyMetadataSchema
...
def create_json_keyfile(private_key, service_key):
    ...
    keyfile = json.dumps(keydata, indent=4, separators=(",", ": "))
    return keyfile


class IssueKeyForm(BaseForm):
    label = _("Issue Service Key")
    description = _(
        "Issue a key to be used for authentication by a service application"
    )
    ...
    @button.buttonAndHandler(_("Issue key"), name="save")
    def handleApply(self, action):
        ...
        msg = _("Key created: ${key_id}", mapping={"key_id": service_key["key_id"]})
        api.portal.show_message(msg, getRequest())

    @button.buttonAndHandler(_("Cancel"), name="cancel")
    def handleCancel(self, action):
        api.portal.show_message(_("Key creation cancelled."), getRequest())
