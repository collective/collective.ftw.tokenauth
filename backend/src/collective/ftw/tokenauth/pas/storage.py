...
class CredentialStorage(object):
    ...
    def get_service_key_for_client_id(self, client_id):
        ...
        keys = [k for k in self._service_keys.values() if k["client_id"] == client_id]
    ...
    def list_service_keys(self, user_id):
        ...
        users_keys = [k for k in self._service_keys.values() if k["user_id"] == user_id]
    ...
    def get_usage_logs(self, key_id, unrestricted=False):
        ...
        entries = sorted(self._usage_logs.get(key_id, []), key=itemgetter("issued"))
        return entries
