...
from collective.ftw.tokenauth.pas.ip_range import permitted_ip
from collective.ftw.tokenauth.service_keys.browser.base_form import valid_ip_range
...
class TestIPRangeParsing(unittest.TestCase):
    def test_single_ipv4_address_is_parsed(self):
        self.assertEqual([ip_network("192.168.0.1")], parse_ip_range("192.168.0.1"))

    def test_single_ipv4_cidr_network_is_parsed(self):
        self.assertEqual(
            [ip_network("192.168.0.0/16")], parse_ip_range("192.168.0.0/16")
        )
...
class TestPermittedIPChecking(unittest.TestCase):
    ...
    def test_allowed_ip_in_multiple_ipv4_cidr_network_ranges(self):
        self.assertTrue(permitted_ip("192.168.5.5", "10.0.0.0/8, 192.168.0.0/16"))
        self.assertTrue(permitted_ip("10.1.5.20", "10.0.0.0/8, 192.168.0.0/16"))
