...
class TestTokenAuthPlugin(FunctionalTestCase):
    ...
    def test_authenticate_credentials_with_valid_token(self):
        creds = {
            ...
            "access_token": build_access_token(
                self.plugin, arguments={"user_id": TEST_USER_ID}
            ).get("token"),
        }
    ...
    def test_authenticate_credentials_rejects_expired_token(self):
        ...
        actual_response = json.loads(self.request.response.getBody())
    ...
    def test_authenticate_credentials_by_fullname(self):
        ...
        self.assertEqual(
            ("jane", "jane_username"), self.plugin.authenticateCredentials(creds)
        )
    ...
    def test_cleans_up_expired_tokens(self):
        ...
        with freeze_time("2018-01-01 15:30:00") as clock:
            token_1 = build_access_token(
                self.plugin,
                arguments={
                    "user_id": TEST_USER_ID,
                },
    ...
    def test_cant_fetch_usage_logs_for_other_users(self):
        ...
        john = api.user.create(email="jane@plone.org", username="john")
