...
PROJECT_NAME=tokenauth
STACK_NAME=tokenauth-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://tokenauth.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t collective/tokenauth-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t collective/tokenauth-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name tokenauth-frontend-acceptance --link tokenauth-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d collective/tokenauth-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name tokenauth-backend-acceptance -d collective/tokenauth-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop tokenauth-frontend-acceptance
	@docker stop tokenauth-backend-acceptance
